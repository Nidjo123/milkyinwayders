...
public class GreenEnemy extends Entity {
	...
	public void die(MilkyInwayders game) {
		...
		for (int i = 0; i < 20; i++) {
			game.entities.add(new Particle(x + game.random.nextInt(6), y + game.random.nextInt(5), 0x1BD902, 0, 1));
		}
	}
	
	public void render(MilkyInwayders game) {
		if (dirCount % 2 == 0) {
			Art.sheet[1][28].draw(x, y, game);
		} else {
			Art.sheet[1][29].draw(x, y, game);
		}
	}
