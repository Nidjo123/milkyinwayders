...
public class BlueEnemy extends Entity {
	...
	public void tick(MilkyInwayders game) {
		...
		if (MilkyInwayders.tickCount % 30 == 0 && !kamikaze) {
			dirCount++;
			if (dirCount % 4 == 0) {
				right = !right;
				left = !left;
		...
		if (game.random.nextInt(400) == 0 && !kamikaze) {
	...
	public void die(MilkyInwayders game) {
		...
		for (int i = 0; i < 20; i++) {
			game.entities.add(new Particle(x + game.random.nextInt(6), y + game.random.nextInt(5), 0x0026FF, 0, 1));
		}
	}
	
	public void render(MilkyInwayders game) {
		if (dirCount % 2 == 0) {
			Art.sheet[3][28].draw(x, y, game);
		} else {
			Art.sheet[3][29].draw(x, y, game);
		}
	}
